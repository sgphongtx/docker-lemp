 ...
 * @author PhongTX
 */
class Core_Crawler_TintucVn extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
         ...
         * Get title
         */
        $titleNode = $dom->query('.article-title .inner h1');
         ...
         * Get lead
         */
        $leadNode = $dom->query('p.news-content-excerpt');
        ...
        $contentNode = $dom->query('div.content-post');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://tintuc.vn','tintuc.vn','www.tintuc.vn');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn tintuc.vn</p>';
        return $response;
    }
