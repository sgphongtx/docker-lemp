 ...
 * @author PhongTX
 */
class Core_Crawler_ThoiTrangBiz extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
        ...
        $contentNode = $dom->query('div[class="td-post-content"]');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://thoitrang.biz','thoitrang.biz','www.thoitrang.biz');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn thoitrang.biz</p>';
        return $response;        
    }
