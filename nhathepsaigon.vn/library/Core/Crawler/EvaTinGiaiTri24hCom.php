 ...
 * @author PhongTX
 */
class Core_Crawler_EvaTinGiaiTri24hCom extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
         ...
         * Get title
         */
        $titleNode = $dom->query('h1.news_tile');
         ...
         * Get lead
         */
        $leadNode = $dom->query('h2.none-short-news');
        ...
        $contentNode = $dom->query('div.content');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://eva.tingiaitri24h.com','eva.tingiaitri24h.com','www.eva.tingiaitri24h.com');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn eva.tingiaitri24h.com</p>';
        return $response;
    }
