 ...
 * @author PhongTX
 */
class Core_Crawler_NgoisaoNet extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
        ...
        $contentNode = $dom->query('div.fck_detail');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://ngoisao.net','ngoisao.net','www.ngoisao.net');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn ngoisao.net</p>';
        return $response;
    }
