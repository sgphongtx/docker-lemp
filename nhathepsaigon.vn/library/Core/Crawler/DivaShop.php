 ...
 * @author PhongTX
 */
class Core_Crawler_DivaShop extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
         ...
         * Get title 
         */
        $titleNode = $dom->query('h3.post-title');
         ...
         * Get lead 
         */
        $leadNode = $dom->query('.post-content-wrap p strong');
        ...
        $contentNode = $dom->query('.post-content-wrap');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://divashop.vn','divashop.vn','www.divashop.vn');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn divashop.vn</p>';
        return $response;
    }
