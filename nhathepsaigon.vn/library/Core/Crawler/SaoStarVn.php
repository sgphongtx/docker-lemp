 ...
 * @author PhongTX
 */
class Core_Crawler_SaoStarVn extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
         ...
         * Get title
         */
        $titleNode = $dom->query('h1.title_detail_news');
         ...
         * Get lead
         */
        $leadNode = $dom->query('.sapo_detail h2');
        ...
        $contentNode = $dom->query('#content_detail');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://saostar.vn','saostar.vn','www.saostar.vn');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn saostar.vn</p>';
        return $response;
    }
