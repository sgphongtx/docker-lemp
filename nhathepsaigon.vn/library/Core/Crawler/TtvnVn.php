 ...
 * @author PhongTX
 */
class Core_Crawler_TtvnVn extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
         ...
         * Get title
         */
        $titleNode = $dom->query('div.detail h1');
         ...
         * Get lead
         */
        $leadNode = $dom->query('h2.description');
        ...
        $contentNode = $dom->query('#newscontent');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://ttvn.vn','ttvn.vn','www.ttvn.vn');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn ttvn.vn</p>';
        return $response;
    }
