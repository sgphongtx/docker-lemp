 ...
 * @author PhongTX
 */
class Core_Crawler_EmDepVn extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
         ...
         * Get lead
         */
        $leadNode = $dom->query('div.sapo .xsubject');
        ...
        $contentNode = $dom->query('div.article-content');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://emdep.vn','emdep.vn','www.emdep.vn');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn emdep.vn</p>';
        return $response;
    }
