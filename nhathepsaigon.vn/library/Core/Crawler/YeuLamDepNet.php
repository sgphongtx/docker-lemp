 ...
 * @author PhongTX
 */
class Core_Crawler_YeuLamDepNet extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
         ...
         * Get title 
         */
        $titleNode = $dom->query('h1.detail-title');
         ...
         * Get lead 
         */
        $leadNode = $dom->query('h2.entry-description');
         ...
         * Get content 
         */                        
        $contentNode = $dom->query('div#main-detail');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://yeulamdep.net','yeulamdep.net','www.yeulamdep.net');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn yeulamdep.net</p>';
        return $response;        
    }
