 ...
 * @author PhongTX
 */
class Core_Crawler_NgoisaoVn extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
         ...
         * Get title
         */
        $titleNode = $dom->query('.tit-baiviet h1');
         ...
         * Get lead
         */
        $leadNode = $dom->query('.tit-baiviet .sapo-chitiet');
        ...
        $contentNode = $dom->query('#detail-content');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://ngoisao.vn','ngoisao.vn','www.ngoisao.vn');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn ngoisao.vn</p>';
        return $response;
    }
