 ...
 * @author PhongTX
 */
class Core_Crawler_Guu extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
         ...
         * Get title 
         */
        $titleNode = $dom->query('h1.small-image-title');
         ...
         * Get lead 
         */
        $leadNode = $dom->query('h2.guu-article-summary');
        ...
        $contentNode = $dom->query('div.guu-article-body');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'https://guu.vn','guu.vn','www.guu.vn');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn guu.vn</p>';
        return $response;        
    }
