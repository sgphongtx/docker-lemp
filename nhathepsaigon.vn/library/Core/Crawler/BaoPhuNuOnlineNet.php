 ...
 * @author PhongTX
 */
class Core_Crawler_BaoPhuNuOnlineNet extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
         ...
         * Get title
         */
        $titleNode = $dom->query('div.leftD h1');
        ...
        $contentNode = $dom->query('div.detail');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://baophunuonline.net','baophunuonline.net','www.baophunuonline.net');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn baophunuonline.net</p>';
        return $response;
    }
