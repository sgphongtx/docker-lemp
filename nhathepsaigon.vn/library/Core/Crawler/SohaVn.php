 ...
 * @author PhongTX
 */
class Core_Crawler_SohaVn extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
         ...
         * Get title
         */
        $titleNode = $dom->query('h1.news-title');
         ...
         * Get lead
         */
        $leadNode = $dom->query('h2.news-sapo');
        ...
        $contentNode = $dom->query('div.news-content');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://soha.vn','soha.vn','www.soha.vn');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn soha.vn</p>';
        return $response;
    }
