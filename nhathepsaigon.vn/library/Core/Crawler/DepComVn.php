 ...
 * @author PhongTX
 */
class Core_Crawler_DepComVn extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
         ...
         * Get title 
         */
        $titleNode = $dom->query('.article-header h1');
         ...
         * Get lead 
         */
        $leadNode = $dom->query('.article-body .summary');
        ...
        $contentNode = $dom->query('.article-body .text');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://dep.com.vn','dep.com.vn','www.dep.com.vn');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn dep.com.vn</p>';
        return $response;        
    }
