 ...
 * @author PhongTX
 */
class Core_Crawler_Pose extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
         ...
         * Get title 
         */
        $titleNode = $dom->query('h1.hLargeTitle');
         ...
         * Get lead 
         */
        $leadNode = $dom->query('div.pullQuote blockquote');
        ...
        $contentNode = $dom->query('div.innerContent');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://pose.com.vn','pose.com.vn','www.pose.com.vn');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn pose.com.vn</p>';
        return $response;        
    }
