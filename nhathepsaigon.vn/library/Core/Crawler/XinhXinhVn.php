 ...
 * @author PhongTX
 */
class Core_Crawler_XinhXinhVn extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
         ...
         * Get title
         */
        $titleNode = $dom->query('.panel-body h1');
        ...
        $contentNode = $dom->query('div.content-detail');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://xinhxinh.vn','xinhxinh.vn','www.xinhxinh.vn');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn xinhxinh.vn</p>';
        return $response;
    }
