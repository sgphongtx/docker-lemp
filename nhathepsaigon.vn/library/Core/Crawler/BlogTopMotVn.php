 ...
 * @author PhongTX
 */
class Core_Crawler_BlogTopMotVn extends Core_Crawler_Abstract
{
    /**
    ...
    public static function parse($html)
    {
         ...
         * Get lead 
         */
        $leadNode = $dom->query('div.entry-content p i');
         ...
         * Get content 
         */                        
        $contentNode = $dom->query('div.entry-content');
        ...
        $response['data']['content'] = preg_replace_callback($pregfind, function ($match) {
            return Core_Utils::setImageContent($match,'http://blog.topmot.vn','blog.topmot.vn','www.blog.topmot.vn');
        }, $response['data']['content']);
        //Remove href
        $response['data']['content'] = preg_replace('#</a>#siU', '', preg_replace('#<a.*>#siU', '', $response['data']['content']));
        $response['data']['content'] .= '<p style="text-align: right;">Nguồn topmot.vn</p>';
        return $response;        
    }
